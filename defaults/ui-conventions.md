...
- Record UI review findings with route, viewport, state, evidence, severity, and owning layer. Retain captures under `.project-meta/project/ui/` only when they help owner review or future comparison; transient captures can remain outside durable project memory.
- Keep a prototype handoff clear about approved versus draft source revisions, synthetic data, simulated actions, and states that could not be rendered. Recheck the same state after a UI implementation fix. A prototype lives in its own disposable `.project-meta/prototype/` folder and only needs to convey flow and layout, not exact fidelity.
