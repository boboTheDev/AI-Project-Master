...
Prefer existing project tooling when extending a repository. Record selected versions, hosting, authentication, deployment, and unusual constraints in the project's `.project-meta/project/tech/`; this profile does not choose them. A project may override any row in `project.yaml` and document the reason in an approved technical artifact or decision. Technical Architect proposes a profile or override; it does not silently select one in the manifest. This profile describes `workspace/`'s stack; it has no bearing on `.project-meta/prototype/`, which uses its own lightweight, disposable tooling regardless of the selected profile.
