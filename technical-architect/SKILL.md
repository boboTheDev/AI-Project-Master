...
Read `.project-meta/project/README.md`, `project.yaml`, `STATUS.md`, and affected approved business and technical artifacts and current decisions when present. Check the underlying revision and approval metadata. If required product behavior is draft or absent, keep dependent architecture provisional and identify the decision that blocks approval. In backfill, observed `workspace/` code is evidence of the existing system, not approval of intended architecture.

## OPTIONAL ARTIFACTS

Read scoped `.project-meta/project/ux/` and `.project-meta/project/database/` artifacts, change records, deployment or runbooks, integration contracts, and existing `workspace/` code when relevant. Read `defaults/tech-stack.md` and `defaults/folder-structure.md` only for the affected choices; `tech_profile: null` selects no global stack.

## ALLOWED READS

The configured global library and relevant managed-project artifacts, `workspace/` code, configuration, and operational evidence, within available permissions.

## ALLOWED WRITES

Scoped `.project-meta/project/tech/` drafts and approved revisions after owner review, and technical impact or handoff text in a relevant `.project-meta/project/changes/` record. Update `project.yaml` for a profile or override only after the owner explicitly selects it, and update `STATUS.md` only as an accurate index. Write implementation code only inside `workspace/`, only when the task separately authorizes that work and the affected intent is settled; project-specific knowledge never goes into the global library.
...
4. **Decide, following the shared decision protocol in SKILL-CONTRACT.md.** For a real choice, weigh the proposed approach against requirements, delivery and operating cost, complexity, reliability, security, compatibility, migration effort, and reversibility where relevant, then present one decisive recommendation and why it fits this project, not an open menu. Name a viable alternative only when it materially changes the tradeoff, and state what evidence or assumption could change the choice. Ask before recommending only when a real blocker applies under the shared protocol; otherwise proceed with the recommendation as a labeled draft. Do not manufacture alternatives for a routine detail.
...
6. **Prepare the review and handoff.** Produce a small, coherent `.project-meta/project/tech/` revision with the chosen approach, changed boundaries and contracts, alternatives, assumptions, risks, rollout or compatibility plan when needed, and verification expectations. Request a draft ADR only when the choice warrants durable rationale. Mastermind presents the exact technical and ADR revisions together with affected artifacts for owner review. After approval, record revision and approval metadata, update any explicitly selected profile or override in `project.yaml`, refresh dependent links and `STATUS.md`, and hand implementers the approved sources and checks. If the owner asked only for planning, stop at the reviewable proposal.

## OUTPUTS

Only needed project-local technical artifacts, usually `.project-meta/project/tech/architecture.md` and, when distinct, a scoped stack, structure, or integration document. Use `templates/artifacts/technical-architecture.md` if helpful. Link a draft or approved ADR for a lasting choice and a CHG record when the change is consequential. Backfill output remains draft until reviewed. A concise implementation handoff names approved sources, affected interfaces and code areas in `workspace/`, rollout constraints, and verification.
