...
2. For a new project, resolve the parent directory and project name, create and configure the root adapters, `.project-meta/`, and an empty sibling `workspace/`, set the working root to that project root, and verify the bootstrap before asking business, product, technical, or UX questions. If the owner invokes brainstorming for a project that does not yet exist, ask only for missing name or location information needed to create it, then bootstrap first.
3. For an existing-project backfill, confirm the current directory is the repository root, then use the helper to restructure it in place: the entire existing repository moves under `workspace/` with `.git`, remotes, history, branches, local Git configuration, and working-tree state preserved; `.project-meta/` is created beside it; and the root adapters are installed outside both repositories. If the repository is already arranged as `workspace/`, retain that layout. Complete this before the full inventory.
...
1. Treat “create a project here,” “start a project,” and “backfill this project” as authorization to perform the corresponding local bootstrap, including the required in-place folder restructure and root adapter installation for backfill. Infer routine mechanics such as the folder slug and existing-project display name when unambiguous. Ask a focused question only when the name, target, repository type, Git identity, or a collision cannot be resolved honestly.
2. Use `mastermind/scripts/bootstrap_project.py` as described in the bootstrap reference. Use `new` mode only for a missing or empty target. Use `existing` mode from a normal standalone Git repository root or from a project root whose repository is already under `workspace/`. The helper completes preflight before writes, stages the metadata repository and its initial commit, and rolls back handled failures. Runtime filesystem permission prompts still apply.
3. Verify the configured project name, stable library ID, ignored device-local library path, initial status, root adapters, and first `.project-meta/` commit. For backfill, verify that Git HEAD, branch, remotes, local configuration, tracked changes, and untracked files are unchanged after the repository moves under `workspace/`. Do not create all specialist directories during initialization.
4. Continue directly to the discovery, brainstorming, status, or backfill outcome in the same request. Project creation is a setup step rather than the requested planning result. If the runtime cannot adopt the new root in place, tell the owner to relaunch it from the project root before continuing.
...
5. Commit `.project-meta/` at meaningful lifecycle checkpoints: a coherent `needs-review` packet, its approved promotion, and verified or closed change state. Include relevant CHG or ADR identifiers. Do not commit routine draft edits automatically and do not make commits in `workspace/` on Project Master's behalf.

## OUTPUTS

For **initialization**, root runtime adapters, a configured and initially committed `.project-meta/`, and a properly sited `workspace/`; for backfill, the existing repository is preserved intact inside `workspace/`, including Git configuration and working state. For **status**, a concise evidence-linked report and a corrected `.project-meta/project/STATUS.md` only when needed. For **brainstorming**, focused questions, a concrete `.project-meta/project/` draft, and an owner review brief when consequential intent is ready. For **change work**, a scoped `CHG-*.md` when warranted, approved artifact and decision updates after review, and an implementation handoff if requested. Specialist outputs remain in their project-local folders.
...
For initialization, the project root has verified `AGENTS.md` and `CLAUDE.md`, a configured `.project-meta/` with its baseline commit, and a `workspace/` that is newly empty or contains the restructured existing repository with unchanged Git history, remotes, branches, local configuration, and working-tree state. The two directories are separate Git repositories, and the runtime is operating from the project root. For a status request, the owner can see the supported state, uncertainty, and next step. For brainstorming, the proposal and approval boundary are concrete and the requested planning outcome is reached. For implementation, the handoff or authorized work is completed and checked. In every mode, `STATUS.md` and decision links match underlying artifacts when updated.
