...
4. After the owner approves the exact artifact and ADR revisions shown, the affected `.project/` artifacts and ADR become approved, the CHG records its reviewed packet, and `STATUS.md` links the change with implementation still `not-started`. The coding agent receives a handoff limited to that scope. If the owner did not review the ADR, it stays draft.
