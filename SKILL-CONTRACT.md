...
1. **Locate the managed project first.** Read its `.project/README.md`, `project.yaml`, and relevant `STATUS.md` entries. Check the underlying artifacts before consequential work; status is an index, not authority.
2. **Respect the write boundary.** During managed-project work, read the global library as needed. Write project knowledge, proposals, decisions, change records, and review evidence only to that project's `.project/`; write implementation only in that project's code directories. Change this library only during intentional Project Master maintenance.
